...
To attempt to break this code:
	...
	material and task names may not contain " - ",
		as this is used in the .txt file to separate information.

Note:
	There is code inside an if( IS_PI ) statement, which will only run if the code is run on a raspberry pi.
	It will then only complete this code if the gpiozero library is installed.
	The gpiozero library will only work on a raspberry pi, and the implementation of this code
	cannot be completed until I create some hardware devices for the pi.
	Notify my of any errors generated by this code, but ignore any warnings, as the warnings are expected.